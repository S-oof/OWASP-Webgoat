...
#### 7. ПОРУШЕННЯ КОНФІДЕНЦІЙНОСТІ, ВИКОРИСТОВУЮЧИ РЯДКОВУ SQL-ІН'ЄКЦІЮ
Використовуючи форму нижче та рядкову SQL-ін'єкцію, спробуємо отримати всі дані про співробітників із таблиці employees:
<img width="1541" height="605" alt="image" src="https://github.com/user-attachments/assets/3d60d14a-d938-43ca-95b3-e68eeb2df5a4" />
#### 8. ПОРУШЕННЯ ЦІЛІСНОСТІ, ВИКОРИСТОВУЮЧИ ЛАНЦЮЖКИ ЗАПИТІВ (Query chaining)
Для виконання цього завдання потрібно в одне з полів (наприклад, у поле прізвища) ввести команду, яка спочатку закриє основний запит, а потім запустить ваш власний запит на оновлення даних (UPDATE).
<img width="698" height="537" alt="image" src="https://github.com/user-attachments/assets/82dd1066-b1e0-4029-969c-af528cba8be4" />
#### 9. ПОРУШЕННЯ ДОСТУПНОСТІ
...
## (A7) Identity and Auth Failure
## Authentication Bypasses

Обхід механізмів автентифікації може здійснюватися різними способами, але зазвичай він базується на використанні недоліків у конфігурації або логіці роботи системи. Це передбачає маніпуляції для створення умов, необхідних для успішного обходу.
Приховані поля введення

Найпростіший варіант — це використання прихованого поля введення на вебсторінці або в DOM-структурі.
Видалення параметрів

Іноді, не знаючи правильного значення параметра, зловмисник може повністю видалити його із запиту, щоб перевірити реакцію системи.
Примусовий перегляд (Forced Browsing)

Якщо певний розділ сайту не має належного захисту на рівні конфігурації, отримати до нього доступ можна шляхом вгадування або перебору (брутфорсу).

#### 2FA Password Reset

The Scenario: 

Ви скидаєте пароль, але робите це з місця або пристрою, які ваш провайдер не розпізнає. Через це вам потрібно відповісти на контрольні запитання, які ви раніше налаштували. Проблема полягає в тому, що інформація про ці запитання зберігається на іншому пристрої (якого немає під рукою), а самі відповіді ви не пам’ятаєте.
